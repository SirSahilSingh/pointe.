...
# ── PyQt6-dependent classes are only defined when PyQt6 is available ──
if _HAS_PYQT:
...
class FloatingHUD:
    ...
    Falls back to no-op if PyQt6 is not installed.
    """
    def __init__(self):
        self.queue = None
        self.process = None
        if not _HAS_PYQT:
            print("[HUD] PyQt6 not available — HUD disabled.")
            return
        try:
            ...
            print(f"[HUD] Failed to start HUD process: {e}")
            self.queue = None
            self.process = None
