        ...
        <div className="animate-in flex flex-col gap-6">
            ...
            <GlassCard hover={false}>
                <div className="flex flex-col gap-4">
                    ...
                    {/* Preset chips — gradient backgrounds */}
                    <div className="grid grid-cols-3 gap-2">
                        {PRESETS.map(preset => (
                            <button
                                key={preset.id}
                                onClick={() => applyPreset(preset.id)}
                                className={`relative flex flex-col items-start gap-1.5 p-3 rounded-xl border transition-all duration-200 overflow-hidden ${activePreset === preset.id
                                    ? 'border-[rgba(255,255,255,0.15)] glow-medium'
                                    : 'border-[rgba(255,255,255,0.04)] hover:border-[rgba(255,255,255,0.08)]'
                                    }`}
                                style={{ background: preset.gradient }}
                            >
                                {/* Gradient underlay decoration */}
                                <div className="absolute top-0 right-0 w-16 h-16 rounded-full opacity-20 blur-xl" style={{ background: preset.accentColor, transform: 'translate(30%, -30%)' }} />

                                <span className="relative z-10">{PRESET_ICONS[preset.id]?.(preset.accentColor)}</span>
                                <span className="relative z-10 text-[12px] font-semibold text-[#f0f0f0] truncate w-full">{preset.name}</span>
                                <span className="relative z-10 text-[10px] text-[#5a5a65] truncate w-full">{preset.description}</span>
                            </button>
                        ))}
                        {/* Custom chip */}
                        <button
                            onClick={() => applyPreset('custom')}
                            className={`relative flex flex-col items-start gap-1.5 p-3 rounded-xl border transition-all duration-200 overflow-hidden ${activePreset === 'custom'
                                ? 'border-[rgba(255,255,255,0.15)] glow-medium'
                                : 'border-[rgba(255,255,255,0.04)] hover:border-[rgba(255,255,255,0.08)]'
                                }`}
                            style={{ background: 'linear-gradient(135deg, rgba(255,255,255,0.04), rgba(255,255,255,0.02))' }}
                        >
                            <span>{PRESET_ICONS.custom('#a0a0a8')}</span>
                            <span className="text-[12px] font-semibold text-[#f0f0f0]">Custom</span>
                            <span className="text-[10px] text-[#5a5a65]">Your own config</span>
                        </button>
                    </div>
